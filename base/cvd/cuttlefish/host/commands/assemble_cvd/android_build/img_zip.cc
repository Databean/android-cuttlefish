...
Result<ImgZip> ImgZip::FromFetcherConfig(const FetcherConfig& config,
                                         FileSource source) {
  BuildArchive archive = CF_EXPECT(FindBuildArchive(config, source, kImgMatch));
  return CF_EXPECT(ImgZip::FromBuildArchive(std::move(archive)));
}

Result<ImgZip> ImgZip::FromDirectory(const std::string& path) {
  BuildArchive archive = CF_EXPECT(FindBuildArchive(path, kImgMatch));
  return CF_EXPECT(ImgZip::FromBuildArchive(std::move(archive)));
}

Result<ImgZip> ImgZip::FromBuildArchive(BuildArchive archive) {
  ImgZip img_zip(std::move(archive));
...
Result<std::set<std::string_view>> ImgZip::Images() {
...
Result<std::string_view> ImgZip::ImageFile(
...
Result<std::map<std::string, std::string>> ImgZip::AndroidInfoTxt() {
  std::string contents = CF_EXPECT(archive_.MemberContents("android-info.txt"));
  return CF_EXPECT(ParseKeyEqualsValue(contents));
}

Result<config::GuestConfigFile> ImgZip::GuestConfigProto() {
  std::string contents =
...
std::ostream& ImgZip::Format(std::ostream& out) const {
  fmt::print(out, "ImgZip {{ {} }}", archive_);
  return out;
}

std::ostream& operator<<(std::ostream& out, const ImgZip& img_zip) {
  return img_zip.Format(out);
}
